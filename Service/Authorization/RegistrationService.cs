...
{
    public class RegistrationService(IUnitOfWorkAuthorization unitOfWork, IMapper mapper, ILoggerFactory logger, IOptions<HashSettings> hashSettings)
    {
        private readonly Hasher _hasher = new(hashSettings);
        private readonly ILogger<RegistrationService> _logger = logger.CreateLogger<RegistrationService>();
        ...
        public async Task<bool> RegistrationUser(UserRequestDto userDto)
        {
            ...
            ICollection<CrmRole> roles = await unitOfWork.Role.GetItems(mapper.Map<User>(userDto).Roles, false);
