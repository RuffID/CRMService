...
namespace CRMService.Infrastructure.DataBase.Repository.Entity
{
    public class TimeEntryRepository(
        ...
        IDeleteItemRepository<TimeEntry, MainContext> delete
    ) : ITimeEntryRepository
    {
        ...
        public void CreateRange(IEnumerable<TimeEntry> entities)
            => create.CreateRange(entities);

        public void Delete(TimeEntry item)
            => delete.Delete(item);

        public void DeleteRange(IEnumerable<TimeEntry> items)
            => delete.DeleteRange(items);
    }
}